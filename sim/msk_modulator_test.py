...
async def my_first_test(dut):
    ...
    dut.clk.value = 0
    dut.init.value = 1
    dut.mod_phase_delta.value = int(bitrate / sample_rate * 2.0**32)
    dut.carrier_phase_delta.value = int(freq_if / sample_rate * 2.0**32)
    dut.tx_data.value = 0
    ...
    while sim_time < 3000:
        ...
        if dut.tx_req.value == 1:
            dut.tx_data.value = random.randrange(1000) % 4
